...
const App = () => {
  ...
  const ChangeHandlerVisit = id => {
    const filteredData = countriesList.map(eachItem => {
      if (eachItem.id === id) {
    ...
    const getData = filteredData.find(eachItem => eachItem.id === id)

    SetCountriesData(prevState => [...prevState, getData])

    SetCountriesList(filteredData)
  }

  const RemoveHandler = id => {
    const RemoveItems = countriesList.map(eachItem => {
      if (eachItem.id === id) {
        return {...eachItem, isVisited: !eachItem.isVisited}
      }
      return eachItem
    ...
    const filteredData = countriesData.filter(eachItem => eachItem.id !== id)

    SetCountriesData(filteredData)
    SetCountriesList(RemoveItems)
  }
        ...
        <ul className="List-container">
          {countriesList.map(eachItem => (
            <CountriesList
              ...
              key={eachItem.id}
              ChangeHandler={ChangeHandlerVisit}
            />
          ))}
        </ul>
